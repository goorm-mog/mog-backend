...
public class MidpointController {
    ...
    // ──────────────────────────────────────────
    // 4. 중간지점 계산 + 소요시간 계산 (방장)
    // ──────────────────────────────────────────
    @Operation(summary = "중간지점 계산", description = "방장이 카카오 모빌리티 API를 활용한 이진 탐색으로 중간지점을 계산합니다. 참여자별 소요시간도 함께 계산되어 저장됩니다.")
    ...
    // 5. 중간지점 조회
    // ──────────────────────────────────────────
    @Operation(summary = "중간지점 조회", description = "계산된 중간지점 좌표와 참여자별 소요시간을 조회합니다. 아직 계산되지 않은 경우 404를 반환합니다.")
    @GetMapping("/midpoint")
